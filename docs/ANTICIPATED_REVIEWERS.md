...
| "Is `R(S)` just author-defined realism?" | `R(S)` is treated as a structural quality-control score. Human ratings show it is not a substitute for human realism judgment, which is why the paper reports human validation separately. | `results/human_realism_validation.json`, `docs/PAPER_VALIDITY_ETHICS_REPRODUCIBILITY.md` |
| "Do detection results depend on simulator assumptions?" | The paper includes SOC-simulation sensitivity analysis and empirical Sigma replay grounding where labelled logs are available. | `results/soc_sensitivity.json`, `results/sigma_measured.json`, `paper_assets/figures/sigma_replay_coverage.png` |
...
| "Does the validation generalize beyond one model?" | The repository has a multi-model runner and a small tracked replication; exact-size replication is wired as `make multimodel-exact-size` and should be run before claiming exact-size results. | `analysis/multimodel.py`, `results/multimodel.json` |
