...
def write_table(result: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "\\begin{table*}[t]",
        "\\centering",
        "\\caption{LLM pipeline versus ATT\\&CK-constrained non-LLM template baseline. Matched-evaluation $n$ is used for comparison; full/source $n$ annotates unequal corpus sizes.}",
        "\\label{tab:non-llm-baseline}",
        "\\small",
        "\\begin{tabular}{lrrrrrr}",
        "\\toprule",
        "Generator & Full/source n & Matched n & Active IDs & Base techniques & Mean $R(S)$ & Detail proxy \\\\",
        "\\midrule",
